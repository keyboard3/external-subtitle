import formats from "./format/index";
import {
  BuildOptions,
  Caption,
  ConvertOptions,
...
import { ResyncFunction, SubsrtInterface } from "./types/subsrt";
...
class Subsrt implements SubsrtInterface {
  ...
  convert = (
    content: string,
    _options: ConvertOptions | string = <ConvertOptions> {},
  ) => {
  ...
  resync = (
    captions: Caption[],
    options: ResyncFunction | number | ResyncOptions = <ResyncOptions> {},
  ) => {
    ...
    if (typeof options === "function") {
      ...
      func = (
        ...
        Math.round(a[0] * ratio + offset),
        Math.round(a[1] * ratio + offset),
      ];
    } else {
