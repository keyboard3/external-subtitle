...
import { Caption, ContentCaption } from "../types/handler";

import { SUBBuildOptions, SUBParseOptions } from "./types/sub";
...
const build = (captions: Caption[], options: SUBBuildOptions) => {
  ...
  for (let i = 0; i < captions.length; i++) {
    ...
    if (!caption.type || caption.type === "caption") {
      const startFrame =
        typeof caption.frame === "object" && caption.frame.start >= 0
          ? caption.frame.start
          : caption.start * fps;
      const endFrame =
        typeof caption.frame === "object" && caption.frame.end >= 0
          ? caption.frame.end
          : caption.end * fps;
...
export { build, detect, FORMAT_NAME as name, parse };
